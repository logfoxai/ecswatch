...
async function emitFailureReport(ctx: CliContext, svc: ServiceSnapshot): Promise<void> {
    ...
    await gh.withGroup('ecswatch: stopped tasks', async () => {
        ...
        try {
            ...
            const stopped = await getRecentStoppedTasks(
                ctx.region, ctx.cluster, ctx.service, svc.primaryTaskDefinitionArn,
            );
    ...
    const stoppedForAnalysis = await safe(
    ...
    const diagnostics = analyze({
        service: svc, stoppedTasks: stoppedForAnalysis, runningTasks: [], targetHealth: tgHealth,
    });
